...
class Builder(gymnasium.Env, gymnasium.utils.EzPickle):
    ...
    def reset(
        ...
        *,
        ...
        state = self.task.obs()
        # print(f"DEBUG: state after reset = {state}")
        key_list = list(state.keys())
        processed_state = self.task.process_obs(state)
        new_key_list = []
        for agent in self.possible_agents:
            new_key_list += list(processed_state[agent].keys())
        # print(f"DEBUG: key_list = {set(key_list)}, new_key_list = {set(new_key_list)}")
        assert set(key_list) == set(new_key_list), 'Obs processing changed the keys!'
    ...
    def step(self, action: dict) -> tuple[np.ndarray, float, float, bool, bool, dict]:
        ...
        if exception:
            ...
            costs = {agent: info[agent]['cost_sum'] for agent in self.possible_agents}
        ...
        if self.terminated or self.truncated:
            try:
                ...
                agent_log(
                    ...
                    {
                        "steps": self.steps,
                        "task_num_steps": self.task.num_steps,
                        "terminated": bool(self.terminated),
                        "truncated": bool(self.truncated),
                        "goal_met": bool(info.get("goal_met", False)),
                    },
                    "H1" if self.truncated else "H3",
                    "train",
                )
            except Exception:
                pass
        ...
        for agents in self.possible_agents:
            ...
            # NOTE: terminate conditions are set in wrappers based on subgoals, 
            # deprecated here
    ...
    def num_agents(self) -> int:
        ...
        return self.task.agent.agent_num

    @property
    def possible_agents(self) -> list[str]:
        """Helper to get possible agents."""
        # print(f"self.task.agent = {self.task.agent}")
