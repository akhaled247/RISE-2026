...
class World:  # pylint: disable=too-many-instance-attributes
    ...
    def build(self):  # pylint: disable=too-many-locals, too-many-branches, too-many-statements
        ...
        # Parse the XML string generated by the agent (dynamic multi-agent XML)
        # This ensures the XML is consistent with the agent's configuration
        if hasattr(self._agent, 'engine') and hasattr(self._agent.engine, 'model'):
            # Try to get the XML string from the agent's model if available
            # (Assumes the agent has a .xml_string or similar attribute)
            xml_string = getattr(self._agent, 'xml_string', None)
            if xml_string is None and hasattr(self._agent, '_generate_multi_agent_xml'):
                # Regenerate XML if not stored
                xml_string = self._agent._generate_multi_agent_xml(self._agent.agent_num)
            self.xml = xmltodict.parse(xml_string)
            self.xml_string = xml_string
        else:
            raise RuntimeError("Agent does not have a valid XML string for parsing.")
        ...
        for i in range(self._agent.agent_num):
            worldbody['body'][i]['@pos'] = convert(
                np.r_[self.agent_xy[i], self._agent.z_height],
            )
            worldbody['body'][i]['@quat'] = convert(
                rot2quat(self.agent_rot[i]),
            )
        ...
        cameras = xmltodict.parse(
            ...
            <camera name="fixedfar" pos="0 -5 10" zaxis="0 -.5 1"/>
            
            </b>""",
        )
        # <camera name="fixedfar" pos="0 -8 8" zaxis="0 -1 1"/>
