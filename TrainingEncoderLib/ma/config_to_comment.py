#!/usr/bin/env python3
"""Convert SafePO MA run config.json files to compact hyperparam comments."""
...
if __package__ is None:
    ...
    from TrainingEncoderLib.ma.train_comment_lib import encode_config
else:
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser = argparse.ArgumentParser(
        description="Convert SafePO MA config.json files to hyperparam comments."
    )
    parser.add_argument(
        "config",
        nargs="*",
        help="Path(s) to config.json, inline JSON, or '-' for stdin",
    )
    ...
    if args.config == ["-"]:
        ...
        if sys.stdin.isatty():
            print(
                "Provide config.json path(s), inline JSON, or pipe JSON to stdin",
                file=sys.stderr,
            )
            return 1
        try:
