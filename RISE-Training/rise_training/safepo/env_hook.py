...
class _UnsqueezeSafetyEnv:
    ...
    @property
    def obs_rms(self):
        from rise_training.cmdp.normalize import find_obs_normalize_wrapper
    ...
    def obs_rms(self, value) -> None:
        from rise_training.cmdp.normalize import apply_rms_normalizer
...
def make_specrlbench_ma_multi_goal_env(task: str, seed: int, cfg_train: dict):
    ...
    # before importing rise_training (parent sys.path is not inherited).
    from rise_training.paths import ensure_specrlbench_paths, specrl_path_roots

    ensure_specrlbench_paths()
    specrl_root, sg_root, safepo_root = specrl_path_roots()
        ...
        def init_env():
            ...
            for p in (specrl_root, sg_root, safepo_root):
                if p not in sys.path:
                    sys.path.insert(0, p)
            from rise_training.safepo.farama_filter import silence_farama_adroit_spam
