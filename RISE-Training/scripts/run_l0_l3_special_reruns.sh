...
# ---------------------------------------------------------------------------
# 1) PPO  PointLTL2MASAR1-v0
# ---------------------------------------------------------------------------
TASK="PointLTL2MASAR1-v0"
ALGO="ppo"
EXP="ppo_l2_special_s${SEED}"
...
# ---------------------------------------------------------------------------
# 2) PPO  PointLTL2MASAR1WC-v0
# ---------------------------------------------------------------------------
TASK="PointLTL2MASAR1WC-v0"
ALGO="ppo"
EXP="ppo_l2wc_special_s${SEED}"
echo "======== TRAIN $ALGO $TASK  exp=$EXP  gpu=$DEVICE_ID ========"
python train/ppo_train_env.py \
  --task "$TASK" --seed "$SEED" \
  --experiment "$EXP" \
  --log-dir "$LOG_ROOT" \
  --total-steps 10000000 --num-envs 8 --steps-per-epoch 65536 \
  --actor-lr 5e-5 --critic-lr 1e-3 \
  --batch-size 256 --learning-iters 10 \
  --target-kl 0.05 --gamma 0.995 --lam 0.98 --lam-c 0.98 \
  --clip-ratio 0.2 --max-grad-norm 40 --hidden-sizes 64 64 \
...
eval_append "$ALGO" "$TASK" "$EXP"
# ---------------------------------------------------------------------------
# 3) PPO-Lag  PointLTL2MASAR1-v0
# ---------------------------------------------------------------------------
TASK="PointLTL2MASAR1-v0"
ALGO="ppo_lag"
EXP="ppo_lag_l2_special_s${SEED}"
echo "======== TRAIN $ALGO $TASK  exp=$EXP  gpu=$DEVICE_ID ========"
python train/ppo_lag_train_env.py \
  ...
  --total-steps 5000000 --num-envs 8 --steps-per-epoch 32768 \
  --cost-limit 0 \
  --lagrangian-multiplier-init 0 --lagrangian-multiplier-lr 1e-3 \
  --actor-lr 5e-5 --critic-lr 1e-3 \
  --batch-size 256 --learning-iters 10 \
  --target-kl 0.05 --gamma 0.995 --lam 0.98 --lam-c 0.98 \
  --clip-ratio 0.2 --max-grad-norm 40 --hidden-sizes 64 64 \
...
eval_append "$ALGO" "$TASK" "$EXP"
# ---------------------------------------------------------------------------
# 4) TRPO  PointLTL1MASAR1-v0
# ---------------------------------------------------------------------------
TASK="PointLTL1MASAR1-v0"
ALGO="trpo"
EXP="trpo_l1_special_s${SEED}"
echo "======== TRAIN $ALGO $TASK  exp=$EXP  gpu=$DEVICE_ID ========"
