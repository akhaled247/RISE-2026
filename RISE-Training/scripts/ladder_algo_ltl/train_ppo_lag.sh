...
run_one() {
  ...
  experiment="$(experiment_for "ppo_lag" "$level" "$variant")"

  echo "======== TRAIN ppo_lag $task  exp=$experiment  gpu=$DEVICE_ID ========"
...
echo "PPO-Lag ladder done (seed=$SEED, log_root=$LOG_ROOT)."
