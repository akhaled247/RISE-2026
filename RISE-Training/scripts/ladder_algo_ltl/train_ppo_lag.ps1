# SafePO PPO-Lag — PointLTL{0,1,3}MASAR1[-WC]-v0 (6 envs, fixed hyperparams).
# Run in its own terminal:  .\scripts\ladder_algo_ltl\train_ppo_lag.ps1
...
foreach ($job in $jobs) {
    ...
    $experiment = Get-LadderExperiment -Algo "ppo_lag" -Level $job.Level -Variant $job.Variant
    Write-Host "======== TRAIN ppo_lag $task  exp=$experiment  gpu=$($env:DEVICE_ID) ========"
...
Write-Host "PPO-Lag ladder done (seed=$($env:SEED), log_root=$($env:LOG_ROOT))."
