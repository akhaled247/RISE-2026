...
# Run in its own terminal:  .\scripts\ladder_algo_ltl\train_trpo.ps1
...
foreach ($job in $jobs) {
    ...
    $experiment = Get-LadderExperiment -Algo "trpo" -Level $job.Level -Variant $job.Variant
    Write-Host "======== TRAIN trpo $task  exp=$experiment  gpu=$($env:DEVICE_ID) ========"
...
Write-Host "TRPO ladder done (seed=$($env:SEED), log_root=$($env:LOG_ROOT))."
