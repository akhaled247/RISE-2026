...
``rise_training.safepo.evaluate`` and mirrors its rollout with an explicit
``deterministic`` flag plus action-norm sanity.

Example (Linux, from RISE-Training root)::
...
# RISE-Training root on sys.path (script lives in scripts/)
...
def _rollout_metrics(
    *,
    ...
    from rise_training.safepo.env_hook import (
        is_specrlbench_env,
...
def main(argv: list[str] | None = None) -> int:
    from rise_training.safepo.evaluate import _load_run_artifacts
