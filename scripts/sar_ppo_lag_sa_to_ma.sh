#!/usr/bin/env bash
# PPO-Lag: train single-agent on MASAR1WC → deploy eval on MASAR2WC.
# Run in its own terminal:
#   ./scripts/sar_ppo_lag_sa_to_ma.sh
#   SEED=0 DEVICE_ID=1 ./scripts/sar_ppo_lag_sa_to_ma.sh
set -euo pipefail

REPO_ROOT="$(cd "$(dirname "$0")/.." && pwd)"
cd "$REPO_ROOT/RISE-Training"

TASK="${TASK:-PointLTL0MASAR1WC-v0}"
EVAL_ENV="${EVAL_ENV:-PointLTL0MASAR2WC-v0}"
SEED="${SEED:-0}"
DEVICE="${DEVICE:-cuda}"
DEVICE_ID="${DEVICE_ID:-1}"
LOG_ROOT="${LOG_ROOT:-./_training_logs/safepo}"
EVAL_EPISODES="${EVAL_EPISODES:-50}"
EXPERIMENT="${EXPERIMENT:-ppo_lag_sar_sa_to_ma_s${SEED}}"

latest_run_dir() {
  local found=""
  found="$(ls -td "${LOG_ROOT}/${TASK}/ppo_lag"/seed-"$(printf '%03d' "${SEED}")"-* 2>/dev/null | head -1 || true)"
  if [[ -z "${found}" ]]; then
    found="$(ls -td "${LOG_ROOT}/${TASK}/ppo_lag"/seed-* 2>/dev/null | head -1 || true)"
  fi
  printf '%s' "${found}"
}

if [[ "${EVAL_ONLY:-0}" != "1" ]]; then
  echo "======== PPO-Lag train ${TASK} seed=${SEED} gpu=${DEVICE_ID} ========"
  python train/ppo_lag_train_env.py \
    --task "$TASK" \
    --seed "$SEED" \
    --experiment "$EXPERIMENT" \
    --log-dir "$LOG_ROOT" \
    --total-steps 10000000 \
    --num-envs 8 \
    --steps-per-epoch 65536 \
    --actor-lr 5e-5 \
    --critic-lr 1e-3 \
    --batch-size 256 \
    --learning-iters 10 \
    --target-kl 0.05 \
    --gamma 0.995 \
    --lam 0.98 \
    --lam-c 0.98 \
    --clip-ratio 0.2 \
    --max-grad-norm 40 \
    --hidden-sizes 64 64 \
    --ent-coef 0.02 \
    --cost-limit 0.0 \
    --lagrangian-multiplier-init 0.0 \
    --lagrangian-multiplier-lr 0.01 \
    --device "$DEVICE" \
    --device-id "$DEVICE_ID" \
    --write-terminal False \
    --use-tensorboard True \
    --parallel True \
    --lr_end_factor 1.0 \
    --sar-ltl-ordering True
fi

if [[ -z "${RUN_DIR:-}" ]]; then
  RUN_DIR="$(latest_run_dir)"
fi
if [[ -z "${RUN_DIR}" || ! -d "${RUN_DIR}" ]]; then
  echo "ERROR: no PPO-Lag run dir under ${LOG_ROOT}/${TASK}/ppo_lag/" >&2
  exit 1
fi
echo "run_dir=${RUN_DIR}"

if [[ "${TRAIN_ONLY:-0}" == "1" ]]; then
  exit 0
fi

echo "======== PPO-Lag SA eval ${TASK} episodes=${EVAL_EPISODES} ========"
python eval_safepo_sa_env.py \
  --run-dir "$RUN_DIR" \
  --eval-episodes "$EVAL_EPISODES" \
  --seed "$SEED" \
  --device "$DEVICE"

echo "======== PPO-Lag MA deploy eval ${EVAL_ENV} episodes=${EVAL_EPISODES} ========"
python eval_safepo_sa_on_ma_env.py \
  --run-dir "$RUN_DIR" \
  --eval-env "$EVAL_ENV" \
  --eval-episodes "$EVAL_EPISODES" \
  --seed "$SEED" \
  --device "$DEVICE"

echo "Done. SA eval summary: ${RUN_DIR}/eval_summary.json"
echo "Done. MA eval summary: ${RUN_DIR}/eval_summary_ma_deploy.json"
